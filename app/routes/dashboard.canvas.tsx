/**
 * The Strategy Canvas — the artefact the consultation is producing, and the only place the
 * approval gate (ARCHITECTURE.md §6.2) can be worked.
 *
 * One route, not three. The build plan lists canvas / roadmap / report separately, but
 * AgentState accretes in stage order: each section below simply appears when its field lands.
 * That makes the page the running record of the consultation, and print CSS away from being
 * the export.
 */
    ...
    return (
      <main className="mx-auto w-full max-w-4xl flex-1 px-6 py-16 text-center">
        <p className="text-sm text-slate-500">
          Your Canvas fills in as the consultation progresses. Finish the discovery interview
          and your Industry Analyst will put the first use cases here.
        </p>
      </main>
    );
  }
      ...
      >
        ...
        {pending === 0 && !architectureStack && (
          <p className="mt-4 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-500">
            Reviewed. Head back to the chat and send a message — your Technical Architect will
            design around the ones you approved.
          </p>
        )}
      </Section>
